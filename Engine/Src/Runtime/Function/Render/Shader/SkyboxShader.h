...
namespace photon
{
	...
	struct SkyboxShaderBlob : public ShaderBlob
	{
	...
	{
	public:
		SkyboxShader(const std::wstring& filepath)
			: Shader(filepath) {
	...
	private:
		...
		std::unordered_map<std::string, SkyboxShaderBlob> m_VariantToShaderBlob;
	};
